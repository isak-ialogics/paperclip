...
export function issueRoutes(
  ...
  router.post("/companies/:companyId/issues", applyCreateIssueStatusDefault, validate(createIssueSchema), async (req, res) => {
    ...
    if (createBody.idempotencyKey) {
      const [existing] = await db.select({ id: issueRows.id }).from(issueRows).where(and(eq(issueRows.companyId, companyId), eq(issueRows.idempotencyKey, createBody.idempotencyKey), notInArray(issueRows.status, ["cancelled"]))).limit(1);
      if (existing) { const full = await svc.getById(existing.id); res.json(full); return; }
    }
